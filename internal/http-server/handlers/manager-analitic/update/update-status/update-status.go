...
type Request struct {
	...
	NewStatus string `json:"new_status" validate:"required,new_status"`
}
...
func New(ctx context.Context, log *slog.Logger, taskSaver TaskSaver) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "http-server.manager-analitic.update.update-status"
		log := log.With(
			slog.String("op", op),
		...
		if err := taskSaver.TaskUpdateStatus(ctx, req.NewStatus, req.TaskID); err != nil {
